...
router.post('/InsertAlgorithm', (req,res) => {
    ...
    [req.body.algorithmInfo,req.body.algorithmFile,req.body.algorithmName],(err,respuesta) =>{
        if(err){
            ...
            res.status(201).send({message:"Algorithm created Successfully"});
        }
    })
})
router.get('/:id',(req,res) =>{
    const id = req.params.id;
    mysqlConnection.query(`select * from algorithm WHERE idAlgorithm=${id}` ,(err,resp,fields)=>{
        if(!err){
            res.json(resp);
            console.log(resp);
        }else{
...
router.put('/:id',(req,res)=>{
    ...
    [req.body.algorithmName,req.body.algorithmInfo,req.body.algorithmFile], (err,resp)=>{
        if(err){
